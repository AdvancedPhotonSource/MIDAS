#!/usr/bin/env bash
# Release a new version of midas-integrate.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-integrate-v${NEW_VERSION}"

echo "=== Releasing midas-integrate v${NEW_VERSION} (mode: ${MODE}) ==="
echo
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_integrate/. Commit or stash first."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_integrate/__init__.py
rm -f pyproject.toml.bak midas_integrate/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_integrate/__init__.py | cut -d'"' -f2)
...
python -m pytest tests/ -q --tb=short || {
    ...
    git checkout -- pyproject.toml midas_integrate/__init__.py
    exit 1
}
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    echo "  git checkout -- pyproject.toml midas_integrate/__init__.py"
    exit 0
fi
...
if [ "$MODE" = "--publish" ]; then
    ...
    gh release create "$TAG" dist/* \
        --title "midas-integrate v${NEW_VERSION}" \
        --generate-notes
    ...
    echo "  PyPI: https://pypi.org/project/midas-integrate/${NEW_VERSION}/"
    echo "  Verify: pip install -U midas-integrate && \\"
    echo "          python -c 'import midas_integrate; print(midas_integrate.__version__)'"
    echo
    echo "Verify:"
    echo "  pip install -U midas-integrate"
    echo "  python -c 'import midas_integrate; print(midas_integrate.__version__)'"
    exit 0
fi
...
echo "    --title 'midas-integrate v${NEW_VERSION}' \\"
echo "    --generate-notes"
echo

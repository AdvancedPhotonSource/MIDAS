...
Single logger named ``midas_ff_pipeline`` to keep child stage logs
...
LOG = logging.getLogger("midas_ff_pipeline")


def configure_logging(level: int = logging.INFO) -> None:
    """Idempotent root-logger setup. Safe to call from CLI or notebook."""
    root = logging.getLogger("midas_ff_pipeline")
    if root.handlers:
        # Already configured (e.g., second pipeline instance in same notebook).
        root.setLevel(level)
        return
...
def stage_timer(stage_name: str) -> Iterator[dict]:
    ...
    The yielded dict can be stashed into a StageResult for provenance.
    """

#!/usr/bin/env bash
# Release a new version of midas-ff-pipeline.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-ff-pipeline-v${NEW_VERSION}"

echo "=== Releasing midas-ff-pipeline v${NEW_VERSION} (mode: ${MODE}) ==="
echo
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_ff_pipeline/. Commit or stash first."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_ff_pipeline/__init__.py
rm -f pyproject.toml.bak midas_ff_pipeline/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_ff_pipeline/__init__.py | cut -d'"' -f2)
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    echo "  git checkout -- pyproject.toml midas_ff_pipeline/__init__.py"
    exit 0
fi

# --- 6. Commit + tag ---
echo "[4/7] Committing version bump..."
git add pyproject.toml midas_ff_pipeline/__init__.py
if git diff --cached --quiet; then
    echo "  Version was already at ${NEW_VERSION} on disk; skipping commit."
else
    git commit -m "midas-ff-pipeline: bump version to ${NEW_VERSION}"
fi

echo "[5/7] Tagging as ${TAG}..."
git tag -a "$TAG" -m "midas-ff-pipeline v${NEW_VERSION}"
...
if [ "$MODE" = "--publish" ]; then
    ...
    gh release create "$TAG" dist/* \
        --title "midas-ff-pipeline v${NEW_VERSION}" \
        --generate-notes
    ...
    echo "  PyPI: https://pypi.org/project/midas-ff-pipeline/${NEW_VERSION}/"
    echo "  Verify: pip install -U midas-ff-pipeline && \\"
    echo "          python -c 'import midas_ff_pipeline; print(midas_ff_pipeline.__version__)'"
    exit 0
fi
...
echo "    --title 'midas-ff-pipeline v${NEW_VERSION}' \\"
echo "    --generate-notes"
echo

# Releasing midas-diffract

This document describes how to cut a new release of the
`midas-diffract` package to PyPI and GitHub.
...
| `./release.sh 0.1.1 --dry-run` | **Prepare but don't commit or tag**. For testing the build. Easy to undo with `git checkout -- pyproject.toml midas_diffract/__init__.py`. |
...
2. Verify the tag `midas-diffract-v0.1.1` does not already exist.
3. Bump the version in `pyproject.toml` and `midas_diffract/__init__.py`.
...
7. Create an annotated git tag `midas-diffract-v0.1.1`.
...
gh release create midas-diffract-v0.1.1 dist/* \
    --title "midas-diffract v0.1.1" \
    --generate-notes
...
```bash
git tag -d midas-diffract-v0.1.1
```

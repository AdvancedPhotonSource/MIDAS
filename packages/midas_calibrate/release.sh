#!/usr/bin/env bash
# Release a new version of midas-calibrate.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-calibrate-v${NEW_VERSION}"

echo "=== Releasing midas-calibrate v${NEW_VERSION} (mode: ${MODE}) ==="
echo
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_calibrate/. Commit or stash first."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_calibrate/__init__.py
rm -f pyproject.toml.bak midas_calibrate/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_calibrate/__init__.py | cut -d'"' -f2)
...
python -m pytest tests/ -q --tb=short || {
    ...
    git checkout -- pyproject.toml midas_calibrate/__init__.py
    exit 1
}
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    echo "  git checkout -- pyproject.toml midas_calibrate/__init__.py"
    exit 0
fi

# --- 6. Commit + tag ---
echo "[4/7] Committing version bump..."
git add pyproject.toml midas_calibrate/__init__.py
if git diff --cached --quiet; then
    echo "  Version was already at ${NEW_VERSION} on disk; skipping commit."
else
    git commit -m "midas-calibrate: bump version to ${NEW_VERSION}"
fi

echo "[5/7] Tagging as ${TAG}..."
git tag -a "$TAG" -m "midas-calibrate v${NEW_VERSION}"
...
if [ "$MODE" = "--publish" ]; then
    ...
    gh release create "$TAG" dist/* \
        --title "midas-calibrate v${NEW_VERSION}" \
        --generate-notes
    ...
    echo "  PyPI: https://pypi.org/project/midas-calibrate/${NEW_VERSION}/"
    echo "  Verify: pip install -U midas-calibrate && \\"
    echo "          python -c 'import midas_calibrate; print(midas_calibrate.__version__)'"
    exit 0
fi
...
echo "    --title 'midas-calibrate v${NEW_VERSION}' \\"
echo "    --generate-notes"
echo

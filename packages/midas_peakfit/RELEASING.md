# Releasing midas-peakfit

This document describes how to cut a new release of the
`midas-peakfit` package to PyPI and GitHub.
...
| `./release.sh 0.1.1 --dry-run` | **Prepare but don't commit or tag**. For testing the build. Easy to undo with `git checkout -- pyproject.toml midas_peakfit/__init__.py`. |
...
2. Verify the tag `midas-peakfit-v0.1.1` does not already exist.
3. Bump the version in `pyproject.toml` and `midas_peakfit/__init__.py`.
...
7. Create an annotated git tag `midas-peakfit-v0.1.1`.
...
gh release create midas-peakfit-v0.1.1 dist/* \
    --title "midas-peakfit v0.1.1" \
...
- PyPI Trusted Publisher configured at
  ...
  - **PyPI project name**: `midas-peakfit`
  - **Owner**: `marinerhemant`
...
```bash
git tag -d midas-peakfit-v0.1.1
```
...
- **Tag prefix mismatch**: the workflow publishes based on tag
  prefix (`midas-peakfit-v*` → midas-peakfit, `midas-stress-v*` →

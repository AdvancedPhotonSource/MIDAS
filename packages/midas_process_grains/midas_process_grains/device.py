...
Mirrors ``midas_index.device`` / ``midas_transforms.device`` so the package
behaves identically across the midas-* family. Env vars are
``MIDAS_PROCESS_GRAINS_DEVICE`` / ``MIDAS_PROCESS_GRAINS_DTYPE``.
"""
...
def resolve_device(user: str | torch.device | None) -> torch.device:
    ...
    if isinstance(user, torch.device):
        return user
    if user is None:
        user = os.environ.get("MIDAS_PROCESS_GRAINS_DEVICE")
    if user is None:
...
def resolve_dtype(
    device: torch.device, user: str | torch.dtype | None
) -> torch.dtype:
    """Pick a torch.dtype. Precedence: arg > env var > per-device default."""
    if isinstance(user, torch.dtype):
        return user
    if user is None:
        user = os.environ.get("MIDAS_PROCESS_GRAINS_DTYPE")
    if user is not None:
        try:

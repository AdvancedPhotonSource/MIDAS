#!/usr/bin/env bash
# Release a new version of midas-transforms.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-transforms-v${NEW_VERSION}"

echo "=== Releasing midas-transforms v${NEW_VERSION} (mode: ${MODE}) ==="
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_transforms/."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_transforms/__init__.py
rm -f pyproject.toml.bak midas_transforms/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_transforms/__init__.py | cut -d'"' -f2)
...
python -m pytest tests/ -q --tb=short -m "not slow and not gpu and not mps" || {
    ...
    git checkout -- pyproject.toml midas_transforms/__init__.py
    exit 1
}
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    ls -1 dist/
    echo "Undo: git checkout -- pyproject.toml midas_transforms/__init__.py"
    exit 0
fi

echo "[4/7] Committing version bump..."
git add pyproject.toml midas_transforms/__init__.py
if git diff --cached --quiet; then
    echo "  Version was already at ${NEW_VERSION}; skipping commit."
else
    git commit -m "midas-transforms: bump version to ${NEW_VERSION}"
fi

echo "[5/7] Tagging as ${TAG}..."
git tag -a "$TAG" -m "midas-transforms v${NEW_VERSION}"
...
if [ "$MODE" = "--publish" ]; then
    ...
    gh release create "$TAG" dist/* \
        --title "midas-transforms v${NEW_VERSION}" \
        --generate-notes
...
echo "  gh release create ${TAG} dist/* --title 'midas-transforms v${NEW_VERSION}' --generate-notes"

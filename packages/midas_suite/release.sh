#!/usr/bin/env bash
# Release a new version of midas-suite.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-suite-v${NEW_VERSION}"

echo "=== Releasing midas-suite v${NEW_VERSION} (mode: ${MODE}) ==="
echo
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_suite/. Commit or stash first."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_suite/__init__.py
rm -f pyproject.toml.bak midas_suite/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_suite/__init__.py | cut -d'"' -f2)
if [ "$PYPROJ_VER" != "$NEW_VERSION" ] || [ "$INIT_VER" != "$NEW_VERSION" ]; then
    echo "ERROR: version bump failed."
    exit 1
fi

# --- 3. Smoke test (no real test suite — meta-package only) ---
echo "[2/7] Smoke test (import + installed())..."
python -c "
import midas_suite
assert midas_suite.__version__ == '${NEW_VERSION}', \
    f'version mismatch: {midas_suite.__version__} vs ${NEW_VERSION}'
status = midas_suite.installed()
print(f'  {len(status)} sub-packages declared')
for k, v in status.items():
    print(f'    {k}: {v if v else \"NOT INSTALLED\"}')" || {
    echo "ERROR: smoke test failed. Aborting."
    git checkout -- pyproject.toml midas_suite/__init__.py
    exit 1
}
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    echo "  git checkout -- pyproject.toml midas_suite/__init__.py"
    exit 0
fi

# --- 6. Commit + tag ---
echo "[4/7] Committing version bump..."
git add pyproject.toml midas_suite/__init__.py
if git diff --cached --quiet; then
    echo "  Version was already at ${NEW_VERSION} on disk; skipping commit."
else
    git commit -m "midas-suite: bump version to ${NEW_VERSION}"
fi

echo "[5/7] Tagging as ${TAG}..."
git tag -a "$TAG" -m "midas-suite v${NEW_VERSION}"
...
if [ "$MODE" = "--publish" ]; then
    ...
    gh release create "$TAG" dist/* \
        --title "midas-suite v${NEW_VERSION}" \
        --generate-notes
    ...
    echo "  1. Run smoke tests on Linux and macOS (Python 3.9/3.11/3.12)"
    ...
    echo "  PyPI: https://pypi.org/project/midas-suite/${NEW_VERSION}/"
    echo "  Verify: pip install -U midas-suite && \\"
    echo "          python -c 'import midas_suite; print(midas_suite.installed())'"
    exit 0
fi
...
echo "    --title 'midas-suite v${NEW_VERSION}' \\"
echo "    --generate-notes"
echo

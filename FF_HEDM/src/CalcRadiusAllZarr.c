...
int main(int argc, char *argv[]) {
  ...
  double Ycen, Zcen, OmegaStep, OmegaFirstFile, Lsd, px, Wavelength, Rsample,
      Hbeam;
  ...
  while ((zip_stat_index(arch, count, 0, finfo)) == 0) {
    ...
    if (strstr(finfo->name,
               "analysis/process/analysis_parameters/DiscModel/0") != NULL) {
      ReadZarrChunk(arch, count, &DiscModel, sizeof(int));
    }
    if (strstr(finfo->name,
               "analysis/process/analysis_parameters/DiscArea/0") != NULL) {
      ReadZarrChunk(arch, count, &DiscArea, sizeof(double));
    }
    ...
    if (strstr(finfo->name, "analysis/process/analysis_parameters/Vsample/0") !=
        NULL) {
      ReadZarrChunk(arch, count, &Vsample, sizeof(double));
    }
  ...
  ReadZarrChunk(arch, locRingThresh, data, dsize);
  int iter;
  ...
  char InputFile[2048];
  sprintf(InputFile, "%s/Result_StartNr_%d_EndNr_%d.csv", Folder, StartNr,
          EndNr);
  FILE *Infile;
  ...
  while (fgets(aline, 1000, hklf) != NULL) {
    sscanf(aline, "%s %s %s %s %d %s %s %s %s %s %lf", dummy, dummy, dummy,
           dummy, &RN, dummy, dummy, dummy, dummy, dummy, &rrd);
    for (i = 0; i < nRings; i++) {
      RingNr = RingNrs[i];
      if (RN == RingNr) {
        RingRads[i] = rrd;
  ...
  while (fgets(aline, 1000, Infile) != NULL) {
    ...
    for (i = 0; i < nRings; i++) {
      if (fabs(rrd - RingRads[i]) < width) {
        ...
        SpotsMat[counter][11] =
            fabs(OmegaStep) + SpotsMat[counter][7] - SpotsMat[counter][6];
    ...
    if (SpotsMat[counter][13] == -1 && found == 1)
      counter--;
  }
  for (i = 0; i < nRings; i++) {
  ...
  for (i = 0; i < counter; i++) {
    ...
    deltaTheta =
        deg2rad *
        (asind(((sind(SpotsMat[i][9])) * (cosd(SpotsMat[i][11]))) +
               ((cosd(SpotsMat[i][9])) * (fabs(sind(SpotsMat[i][10]))) *
                (sind(SpotsMat[i][11])))) -
         SpotsMat[i][9]);
    SpotsMat[i][14] = 0.5 * ((double)mhkl[ctr]) * deltaTheta *
                      cosd(SpotsMat[i][9]) * Vgauge * SpotsMat[i][1] /
                      (SpotsMat[i][12] * PowderInt[ctr]);
